const KcAdminClient = require('keycloak-admin-client').default;
...
class KeycloakService {
  ...
  async authenticateUser(username, password, clientType = 'farmer') {
    try {
      ...
      // Get user profile
      const userProfile = await this.getUserProfile(decodedToken.sub);
  ...
  async verifyToken(token) {
    try {
      ...
      const verified = jwt.verify(token, signingKey, {
        ...
        audience: Object.values(this.clientConfigs).map(c => c.clientId),
      });
